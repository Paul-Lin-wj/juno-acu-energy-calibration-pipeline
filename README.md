...
❌            连续谱核素挑选                          【备谱 code 暂缺；暂以已有数据
   │                                                  作外部输入（契约 + 清单）】
   ▼
   Run{N}_SelectionResult.npz / correlation_result_RUN{N}.npz（fitter 输入）
   ▼  fitter/
...
`--launched-by agent`），顶层 `run_pipeline.sh` 已扩展为 esd2npz → fitter →
...
fitter 自动读取**同批** esd2npz 的 `results/selection_npz`，nlfit 自动读取
**同批** fitter 的 `results/`——全链无需手动衔接。
