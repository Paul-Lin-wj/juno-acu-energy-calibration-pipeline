...
stages this module runs (run-info, load, fv, correlate, save).
摘自 nlfit/src/run_logger.py（同仓库同契约）。
"""
...
_CODE_GLOBS = ["config/*.py", "src/*.py", "pipeline/*.py",
               "run_pipeline.sh", "setup_env.sh", "calib_run_info/*"]
...
class RunLogger:
    ...
    def snapshot_config(self):
        """Fingerprint the effective config (paths.py + run inputs + env)."""
        import config.paths as P  # noqa: PLC0415
        cfg = {
            "input_data_dir": str(P.INPUT_DATA_DIR),
            "calib_info_file": str(P.CALIB_INFO_FILE),
            "calib_pos_file": str(P.CALIB_POS_FILE),
            "vertex_algo": P.VERTEX_ALGO,
            "fv_energy_region": P.FV_ENERGY_REGION,
            "params_neutron": P.PARAMS_NEUTRON,
            "params_standard": P.PARAMS_STANDARD,
            "sources_do_correlation": P.SOURCES_DO_CORRELATION,
            "sources_do_cs137": P.SOURCES_DO_CS137,
            "cs137_isolation_time": P.CS137_ISOLATION_TIME,
            "cs137_energy_std_cut": P.CS137_ENERGY_STD_CUT,
            "cwd": os.getcwd(),
        }
