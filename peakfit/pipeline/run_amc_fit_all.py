...
输入：calibsel AmC 支线的挑选结果 correlation_result_RUN{N}.npz
...
Usage:
    .venv/bin/python pipeline/run_amc_fit_all.py --run 10104 \
        --corr-npz <calibsel_amc>/results/RUN10104/correlation_result_RUN10104.npz \
...
def load_amc_energies(corr_npz: Path) -> dict:
    """从关联挑选结果取三段能量（键名即 calibsel AmC 支线输出约定，未做变换）。"""
...
def main() -> int:
    ...
    ap.add_argument("--corr-npz", required=True,
                    help="correlation_result_RUN{N}.npz from calibsel AmC branch")
