...
> **AmC / O16** (validated 2026-08-27): `pipeline/run_amc_fit_all.py` fits the nH/nC pure Gaussians (`src/NhnCFitter.py`) and the O16 template decomposition (`fitters/O16Fitter.py`) from `calibsel`'s `correlation_result_RUN{N}.npz`. Note: the O16 minuit HESSE may fail (params at limits) — `fit_valid` is recorded in run_log; the point estimate is used with `MU_ERR_FLOOR` downstream.
