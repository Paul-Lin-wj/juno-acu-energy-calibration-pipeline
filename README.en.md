...
`nlfit/dybmodel/` (pinned to the upstream `wujxy/ENL_fitter` commit), its
runtime data set ships with the repo (small files in plain git, Quenching.root
+ prebuilt binary via Git LFS with sha256 pins; full spectra / repair via the
fetch script), and `nlfit` drives it through a file-feeding sandbox with zero
code changes.
...
| `nlfit/` | Global nonlinearity fit (Stages 4b/5/6/7 + `--phase` per-phase mode): external-data contract, 7-peak aggregation to `gamma_AllPhase.dat`, dybmodel C++ wrap (**source vendored in-repo** at `nlfit/dybmodel/`; runtime data set ships with the repo — small files in plain git, Quenching.root + prebuilt binary via Git LFS, sha256-pinned; sandbox feeds files with zero code changes + behaviour lock), E_rec→E_true inversion lookup; `external_inputs/` holds the interim external-data contract |
