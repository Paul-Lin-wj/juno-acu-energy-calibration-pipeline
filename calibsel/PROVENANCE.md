# standalone_calibsel 代码溯源与环境变动说明
...
## 0b. 2026-08-27 改名与 AmC 支线并入补记（不影响任何算法）

- 模块更名：`esd2npz` → **`calibsel`**（同仓 `fitter` → `peakfit`）。目录更名
  不改文件内容，下表 md5 仍然有效；算法文件内残留的 "standalone_esd2npz"
  字样为历史注释，保留原样以维持 md5 对照。
- AmC 关联对挑选支线并入：原独立模块 `amcsel/` 解散，原样算法文件移至
  `src/amc/`，编排入口 `pipeline/run_amcsel_all.py`，参数并入
  `config/paths.py`（`AMC_*` 常量）——溯源见 **PROVENANCE.amc.md**。
- `pipeline/run_logger.py` 增加一处排除：代码快照跳过 `input/amc_data/`
  的原始 run npz（数据非算法，sha256 由该目录 MANIFEST.sha256 承担）。
- `requirements.txt` 追加 AmC 支线依赖（scienceplots/iminuit/awkward/numba）。
...
md5sum juno_calibration_acu_gamma_source/Finalcorrection_from_npzESD/input/correction_api.py \
       standalone_calibsel/input/correction/correction_api.py
md5sum juno_calibration_acu_gamma_source/calib_run_info/calib_to_analyze.txt \
       standalone_calibsel/calib_run_info/calib_to_analyze.txt
md5sum juno_calibration_acu_gamma_source/calib_run_info/CalibRUN_from_file.csv \
       standalone_calibsel/calib_run_info/CalibRUN_from_file.csv
# (2) 7 个修正数据文件逐字节
...
# (7) venv 版本
standalone_calibsel/.venv/bin/python -c "import sys,numpy,pandas,scipy,matplotlib,uproot;print(sys.version.split()[0]);[print(m.__name__,m.__version__) for m in (numpy,pandas,scipy,matplotlib,uproot)]"
# (8) ROOPY 版本
