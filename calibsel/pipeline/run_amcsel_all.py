#!/usr/bin/env python3
"""
run_amcsel_all.py — calibsel 的 AmC 支线（Stage 3c：(prompt,delayed) 关联对挑选）。

流程与原作者驱动 correlate_selection/test.py（2026-04-14 版）逐步一致，
仅把路径/CLI/留档换成仓库惯例（RunLogger 审计、--launched-by agent 标志）。
分析参数全部来自 config/paths.py 的 AMC_* 常量（逐字节取自 test.py 原值）。
原算法文件在 src/amc/（除 2 行 import 本地化外逐字节原样，见
PROVENANCE.amc.md）。

    Stage 1  run-info   RunManager 查 Source/BKG/位置
    Stage 2  load       BaseAnalyzer.load_data（刻度源 + 本底 npz）
    Stage 3  fv         optimize_fv_cuts + apply_fv_selection + save_fv_cuts
    ...
    Stage 5  save       save_timestamps + plot_correlation_results
                        ...
                        → correlation_result_RUN{N}.npz（供 peakfit 三峰拟合）
...
for _p in ("config", "src", "src/amc", "pipeline"):
...
# 原作者模块（src/amc/）—— 除 import 行本地化外逐字节原样
...
def build_config(run_id: int, input_dir: Path, out_dir: Path) -> AnalysisConfig:
    ...
    return AnalysisConfig(
        ...
        output_base_dir=str(out_dir / "results" / f"RUN{run_id}"),
        timestamp_dir=str(out_dir / "timestamps"),
        vertex_chose=P.AMC_VERTEX_ALGO,
    )


def main() -> int:
    ap = argparse.ArgumentParser(description="calibsel AmC (correlated-pair) "
                                             "orchestrator")
    ap.add_argument("--run", type=int, required=True, help="RUN number")
    ap.add_argument("--input-dir", default=str(P.AMC_INPUT_DATA_DIR),
                    help="directory with Finalcorrection RUN<N>.npz (+BKG)")
    ap.add_argument("--out-dir", default=None,
                    help="output root (default: calibsel/output/<timestamp>)")
    ...
    audit_failed = False
    with RunLogger(out, _PROJ, launched_by=args.launched_by) as logger:
        tee = logger.ConsoleTee(sys.stdout, logger)
        with contextlib.redirect_stdout(tee):
            logger.set_pipeline_info(amc_run=args.run,
            ...
            if run_info is None:
                logger.add_error("1 run-info",
                                 f"run {args.run} not in {P.CALIB_INFO_FILE}")
                logger.set_exit_code(1)
                raise RuntimeError(f"run {args.run} not found in calib info")
            ...
            stages.append({"stage": "run-info", "status": "ok",
            ...
            analyzer.fv_energy_region = P.AMC_FV_ENERGY_REGION
            if not analyzer.load_data():
                logger.add_error("2 load", "load_data returned False")
                logger.set_exit_code(1)
                raise RuntimeError("load_data returned False")
            stages.append({"stage": "load", "status": "ok",
                           "elapsed_s": round(time.time() - t0, 1),
                           "input_dir": str(input_dir)})
            print(f"[Stage2] loaded from {input_dir}")
            ...
            stages.append({"stage": "fv", "status": "ok",
                           "elapsed_s": round(time.time() - t0, 1)})

            # ---------------- Stage 4: correlation ----------------
            t0 = time.time()
            src = config.calib_type
            params = (P.AMC_PARAMS_NEUTRON if ("Am" in src or "Cf" in src)
                      else P.AMC_PARAMS_STANDARD)
            print(f"[Stage4] mode="
                  f"{'neutron' if params is P.AMC_PARAMS_NEUTRON else 'standard'}"
                  f" params={params}")
            analyzer.set_parameters(**params)
            correlation_result = None
            if src in P.AMC_SOURCES_DO_CORRELATION:
                correlation_result = analyzer.find_correlated_events()
            else:
                print(f"[Stage4] source '{src}' not in correlation list, skip")
            if src in P.AMC_SOURCES_DO_CS137:
                print(f"[Stage4] Cs137 isolation "
                      f"±{P.AMC_CS137_ISOLATION_TIME}us, "
                      f"cut Peak+{P.AMC_CS137_ENERGY_STD_CUT}*Std")
                analyzer.find_cs137_events(
                    isolation_time=P.AMC_CS137_ISOLATION_TIME,
                    n_std_cut=P.AMC_CS137_ENERGY_STD_CUT)
            stages.append({"stage": "correlate",
                           "status": "ok",
                           "elapsed_s": round(time.time() - t0, 1),
                           "params": params,
            ...
            select_dir = Path(config.select_result_path)
            corr_npz = select_dir / f"correlation_result_RUN{args.run}.npz"
            if src in P.AMC_SOURCES_DO_CORRELATION and not corr_npz.is_file():
                logger.add_error("5 save", "correlation_result npz missing")
                logger.set_exit_code(1)
                raise RuntimeError("correlation_result npz missing")

            # ---------------- per-run record + audit ----------------
            n_events = {}
            if corr_npz.is_file():
                import numpy as np  # noqa: PLC0415
                with np.load(corr_npz, allow_pickle=True) as d:
                    n_events = {k: int(len(d[k])) for k in d.files}
            if corr_npz.is_file():
                logger.add_output(corr_npz, "correlation_result_npz")
                logger.add_output(select_dir / f"Correlation_RUN{args.run}.pdf",
                                  "correlation_figure")
            stages.append({"stage": "save",
                           "status": "ok" if corr_npz.is_file() else "skipped",
                           "elapsed_s": round(time.time() - t0, 1)})
            logger.add_run(run=args.run, status="ok", source=src,
                           run_info={"bkg_run": run_info["BKG_RUN"],
                                     "position_cm":
                                         [run_info["X(cm)"], run_info["Y(cm)"],
                                          run_info["Z(cm)"]]},
                           stages=stages)
            ...
            expected = [out / "run_log.json", out / "run_log.md",
                        ...
                        select_dir / f"xyz_distribution_RUN{args.run}.pdf",
                        select_dir / f"fv_cuts_RUN{args.run}.npz",
                        out / "timestamps"]
            if corr_npz.is_file():
                expected += [corr_npz,
                             select_dir / f"Correlation_RUN{args.run}.pdf"]
            audit = logger.run_audit(expected)
            if audit["passed"]:
                print(f"[AUDIT] PASSED ({audit['code_snapshot']['n_files']} "
    ...
    print(f"\n[Info] calibsel AmC complete. Output: {out}")
