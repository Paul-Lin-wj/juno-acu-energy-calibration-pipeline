"""
run_logger.py — audit-grade run logging for the standalone_calibsel pipeline.
...
class RunLogger:
    ...
    def _iter_source_files(self):
        ...
        for path in sorted(self.project_root.rglob("*")):
            ...
            if rel.parts[0] in excludes:
                continue
            # AmC 支线的原始 run npz 输入（input/amc_data/，~100MB 级）是
            # 数据不是算法文件：不入代码快照（sha256 由其 MANIFEST.sha256
            # 承担，见 PROVENANCE.amc.md §3）
            if "amc_data" in rel.parts:
                continue
    ...
    def _markdown(self) -> str:
        ...
        L = [f"# Run Log — standalone_calibsel (schema {SCHEMA_VERSION})", "",
